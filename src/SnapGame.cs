    ...
    public class SnapGame
    {
		...
		private static void HandleUserInput(Snap myGame)
		{
			...
			if (SwinGame.KeyTyped (KeyCode.vk_SPACE))
			{
			 ...
			 if (myGame.IsStarted) 			 { 				   if ( SwinGame.KeyTyped (KeyCode.vk_LSHIFT) &&      SwinGame.KeyTyped (KeyCode.vk_RSHIFT)) 				   { 				    //TODO: add sound effects 				   } 				   else if (SwinGame.KeyTyped (KeyCode.vk_LSHIFT)) 				   { 				  	 myGame.PlayerHit ( 0); 				   } 				   else if (SwinGame.KeyTyped (KeyCode.vk_RSHIFT)) 				   { 				  	  myGame.PlayerHit ( 1); 				   }
   			  } 
		}
